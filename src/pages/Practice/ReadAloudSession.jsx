                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div className="p-8">
                        ...
                        <div className="relative py-8">
                            ...
                            {/* Result View - Flash Screen Overlay */}
                            {status === 'result' && result && (
                                <>
                                    {/* Backdrop */}
                                    <div className="fixed inset-0 bg-black/60 z-40 backdrop-blur-sm transition-opacity" onClick={() => { /* Optional: Click outside to close? */ }} />

                                    {/* Modal Container */}
                                    <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[70vw] h-[70vh] bg-white rounded-3xl shadow-2xl z-50 overflow-hidden flex flex-col animate-in zoom-in-50 duration-300">
                                        ...
                                        <div className="px-8 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button onClick={resetSession} className="text-slate-500 hover:text-primary-600 font-semibold text-sm px-4 py-2 hover:bg-slate-100 rounded-lg transition-colors">
                                                    Try Again
                                                </button>
                                                <button onClick={handleNext} className="bg-primary-600 hover:bg-primary-700 text-white font-semibold text-sm px-6 py-2 rounded-lg shadow-md hover:shadow-lg transition-all">
                                                    Next Question
                                                </button>
                                            </div>
                                        </div>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start mb-8">
                                                ...
                                                <div className="bg-white border server-slate-100 rounded-3xl p-8 shadow-sm flex flex-col items-center justify-center relative overflow-hidden">
                                                    <div className="relative w-56 h-56">
                                                        <svg className="w-full h-full transform -rotate-90">
                                                            ...
                                                            <circle
                                                                ...
                                                                strokeLinecap="round"
                                                                strokeDasharray="565"
                                                                strokeDashoffset={565 - (565 * (result.score / 30))}
                                                                className="transition-all duration-1500 ease-out"
                                                            />
                                                        </svg>
                                                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                                                            <span className="text-6xl font-black text-slate-800">{result.score || 0}</span>
                                                            <span className="text-xl text-slate-400 font-medium">/ 30</span>
                                                        </div>
                                                    </div>
                                                    <div className="mt-6 flex items-center gap-2 bg-blue-50 px-4 py-2 rounded-full">
                                                        ...
                                                        <span className="font-bold text-slate-800 ml-2">{result.score.toFixed(2)}</span>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="grid grid-cols-3 gap-4 h-full content-center">
                                                    ...
                                                    <div className="bg-white border border-blue-100 rounded-2xl p-4 flex flex-col justify-between hover:shadow-md transition-shadow">
                                                        ...
                                                        <div className="mt-2 flex items-baseline gap-1">
                                                            <span className="text-2xl font-bold text-slate-800">{result.content}</span>
                                                    ...
                                                    <div className="bg-white border border-red-100 rounded-2xl p-4 flex flex-col justify-between hover:shadow-md transition-shadow">
                                                        ...
                                                        <div className="mt-2 flex items-baseline gap-1">
                                                            <span className="text-2xl font-bold text-slate-800">{result.pronunciation}</span>
                                                            <span className="text-sm text-slate-400">/5</span>
                                                        </div>
                                                        <span className="text-green-600 text-xs font-bold bg-green-50 px-2 py-1 rounded inline-block mt-2 w-fit">Good</span>
                                                    </div>
                                                    ...
                                                    <div className="bg-white border border-orange-100 rounded-2xl p-4 flex flex-col justify-between hover:shadow-md transition-shadow">
                                                        ...
                                                        <div className="mt-2 flex items-baseline gap-1">
                                                            <span className="text-2xl font-bold text-slate-800">{result.fluency}</span>
                                                            <span className="text-sm text-slate-400">/5</span>
                                                        </div>
                                                        <span className="text-green-600 text-xs font-bold bg-green-50 px-2 py-1 rounded inline-block mt-2 w-fit">Good</span>
                                                    </div>
                                            ...
                                            <div className="mb-8">
                                                ...
                                                <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
                                                    <p className="text-lg leading-relaxed text-slate-700">
                                                        {(result.wordAnalysis || question.text.split(' ').map(w => ({ word: w, status: 'good' }))).map((item, index) => {
                                                            let colorClass = "decoration-green-400"; // Default Good
                                                            let textColor = "text-slate-700";

                                                            if (item.status === 'bad') {
                                                                colorClass = "decoration-red-400 decoration-wavy"; // Bad
                                                                textColor = "text-red-600";
                                                            } else if (item.status === 'average') {
                                                                colorClass = "decoration-yellow-400 decoration-wavy"; // Average
                                                                textColor = "text-yellow-700";
                                                            }

                                                            return (
                                                                <span key={index} className={`mr-1.5 inline-block underline underline-offset-4 ${colorClass} ${textColor} hover:bg-slate-100 rounded px-0.5 cursor-pointer transition-colors`}>
                                                                    {item.word}
                                                                </span>
                                                            );
                                                        })}
                                                ...
                                                <div className="flex flex-wrap items-center gap-4 mt-6">
                                                    <div className="flex items-center gap-3 bg-slate-800 text-white px-4 py-2 rounded-xl">
                                                        <div className="w-6 h-6 rounded-full bg-slate-600 flex items-center justify-center font-bold text-xs">{Math.floor(question.text.split(' ').length * 0.7)}</div>
                                                    ...
                                                    <div className="flex items-center gap-3 bg-yellow-50 text-yellow-800 px-4 py-2 rounded-xl border border-yellow-100">
                                                        <div className="w-6 h-6 rounded-full bg-yellow-200 flex items-center justify-center font-bold text-xs">{Math.floor(question.text.split(' ').length * 0.15)}</div>
                                                    ...
                                                    <div className="flex items-center gap-3 bg-red-50 text-red-800 px-4 py-2 rounded-xl border border-red-100">
                                                        <div className="w-6 h-6 rounded-full bg-red-200 flex items-center justify-center font-bold text-xs">{Math.floor(question.text.split(' ').length * 0.15)}</div>
